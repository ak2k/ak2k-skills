...
def _refresh_with(gets, post_payload, unwritable_token=False):
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        orig = (ac.CLIENT_PATH, ac.TOKEN_PATH, ac.httpx.Client)
        ac.CLIENT_PATH = client_path
        ac.TOKEN_PATH = token_path
        ac.httpx.Client = _RecordingClient
        try:
            result = ac._refresh_token({"refresh_token": "stale-token"})
            saved = json.loads(token_path.read_text()) if token_path.exists() else None
            return result, saved
        finally:
            ac.CLIENT_PATH, ac.TOKEN_PATH, ac.httpx.Client = orig
...
def test_scalar_exchange_response_is_rejected():
    ...
    got, _ = _refresh_with([{"token_endpoint": TOKEN_ENDPOINT}], "access_token")
    assert got is None


def test_unusable_discovery_metadata_is_a_spent_grant():
    """AS metadata without a token_endpoint raises KeyError inside
    _exchange_token — a LookupError, so it lands in the re-auth path."""
    got, _ = _refresh_with([{}], {})
    assert got is None
