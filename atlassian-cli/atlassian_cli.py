"""Atlassian Remote MCP CLI — dynamic client for Atlassian's MCP server.

Talks to Atlassian's official Remote MCP server at https://mcp.atlassian.com/v1/mcp
over Streamable HTTP. Discovers tools at runtime via MCP's tools/list, so it
adapts when Atlassian adds or changes tools. Auth is OAuth 2.1 with PKCE +
RFC 7591 dynamic client registration.

Forked from krisp-cli with three substantive changes:

1. MCP_URL points at Atlassian's recommended /v1/mcp endpoint (Streamable HTTP).
   Atlassian still serves /v1/sse for legacy clients but recommends /mcp.

2. OAuth discovery is direct: Atlassian doesn't expose Protected Resource
   Metadata (RFC 9728), so we fetch Authorization Server Metadata (RFC 8414)
   straight from /.well-known/oauth-authorization-server on the MCP host.
   That metadata contains registration_endpoint, authorization_endpoint, and
   token_endpoint, all hosted under cf.mcp.atlassian.com / mcp.atlassian.com.

3. Different REDIRECT_PORT (19877) so atlassian-cli auth and krisp-cli auth
   can run concurrently without colliding on the loopback callback.

Cloudflare-bypass code is preserved verbatim; mcp.atlassian.com is also
Cloudflare-fronted, so direct POSTs may get challenge-page'd on some networks.
"""
...
AS_METADATA_URL = "https://mcp.atlassian.com/.well-known/oauth-authorization-server"
CONFIG_DIR = Path.home() / ".config" / "atlassian"
...
<p>Completing authentication with Atlassian...</p>
<script>
fetch("{url}", {{
  method: "POST",
...
def _discover_oauth() -> dict:
    """Fetch Atlassian MCP's Authorization Server Metadata (RFC 8414).

    Atlassian doesn't publish Protected Resource Metadata (RFC 9728), so the
    krisp-cli two-hop discovery (PRM -> AS metadata) doesn't apply. The MCP
...
def _get_token() -> str:
    ...
    if not data:
        click.echo("Not authenticated. Run: atlassian-cli auth", err=True)
        raise SystemExit(1)
    ...
    click.echo("Token expired. Run: atlassian-cli auth", err=True)
    raise SystemExit(1)
    ...
    def _initialize(self) -> None:
        self._post(
            "initialize",
            {
                ...
                "capabilities": {},
                "clientInfo": {"name": "atlassian-cli", "version": "0.1.0"},
            },
        )
...
@click.group()
def main():
    """Atlassian MCP CLI — dynamic client for Atlassian's Remote MCP server."""


@main.command()
def auth():
    """Authenticate with Atlassian via OAuth PKCE (3LO consent flow)."""
    meta = _discover_oauth()
...
def call(tool_name: str, args_json: str):
    ...
    Examples:
      ...
      atlassian-cli call getJiraIssue '{"cloudId": "...", "issueIdOrKey": "PROJ-1"}'

      atlassian-cli call searchJiraIssuesUsingJql '{"cloudId": "...", "jql": "assignee = currentUser()"}'

      atlassian-cli call getConfluencePage '{"cloudId": "...", "pageId": "12345"}'
    """
    try:
...
def status():
    ...
    data = _load_json(TOKEN_PATH)
    if not data:
        click.echo("Not authenticated. Run: atlassian-cli auth")
        return
    ...
    if remaining > 0:
        ...
        if _refresh_token(data):
            ...
            click.echo("Token expired. Run: atlassian-cli auth")
